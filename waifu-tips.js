...
function loadWidget(config) {
	...
	let userAction = false,
		userActionTimer,
		messageTimer,
		messageArray = ["Long time no see, life flies so fast...", "Big villain! How long have you been ignoring people, huh, huh~", "Hi~ Come and tease me!", "Take a small punch to your chest!", "Remember to add Xiaojia to the Adblock whitelist!"];
	...
	(function registerEventListener() {
		...
		document.querySelector("#waifu-tool .fa-camera-retro").addEventListener("click", () => {
			showMessage("It’s taken, isn’t it cute?", 6000, 9);
		...
		document.querySelector("#waifu-tool .fa-times").addEventListener("click", () => {
			...
			showMessage("I hope you can meet important people again one day.", 2000, 11);
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // If it is the homepage
			...
			else if (now > 13 && now <= 17) text = "It's easy to get sleepy in the afternoon. Have you achieved today's sports goal?";
			...
			else if (domain === "baidu") text = `Hello! Friends from Baidu Search<br>You are searching <span>${referrer.search.split("&wd=")[1].split("&")[0]}</span> Did you find me? `;
			else if (domain === "so") text = `Hello! Friends from 360search<br>You are searching <span>${referrer.search.split("&q=")[1].split("&")[0]}</span> 找到的我吗？`;
			else if (domain === "google") text = `Hello! Friends from Google Search<br>Welcome to read<span>「${document.title.split(" - ")[0]}」</span>`;
			else text = `Hello! From <span>${referrer.hostname}</span> friend`;
		} else {
	...
	function showHitokoto() {
		...
		fetch("https://v1.hitokoto.cn")
			...
			.then(result => {
				const text = 'This sentence comes from <span>「${result.from}」</span>，It was submitted by <span>${result.creator}</span>  hitokoto.cn .`;
	...
	(function initModel() {
		...
		if (modelId === null) {
			...
			modelId = 1; // Model ID
			modelTexturesId = 53; // Model Textures ID
		}
		...
		if (useCDN) {
			...
			fetch(`${apiPath}rand_textures/?id=${modelId}-${modelTexturesId}`)
				...
				.then(result => {
					...
					else loadModel(modelId, result.textures.id, "Does my new dress look good?");
				});
		}
	}
